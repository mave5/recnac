...
experiment='luna_nodule_seg_fulldata'+'_hw_'+str(h)+'by'+str(w)+'_cin'+str(c_in)
...
if os.path.exists(weightfolder+'/train_test_indx.npz'):
    tt_ind=np.load(weightfolder+'/train_test_indx.npz')
    ss_train=tt_ind['train']    
    ss_test=tt_ind['test']    
    print 'train test indices loaded!'
else:
...
# non-cancer dsb data
path2dsbnc_train=path2luna+'dsb_nc_train.hdf5'    
if os.path.exists(path2dsbnc_train):
    ff_dsbnc_r=h5py.File(path2dsbnc_train,'r')
    X_train2=ff_dsbnc_r['X_train']
    Y_train2=ff_dsbnc_r['Y_train']
    print 'hdf5 loaded '
else:    
    # load images and masks    
    ff_dsb_r=h5py.File(path2dsb,'r')
    X=[]
    Y=[]
    for idk in ff_dsb_r.keys():
        if ff_dsb_r[idk].attrs['cancer']==0:
            print idk           
            X0=ff_dsb_r[idk]
            n=int(0.05*X0.shape[0]/c_in)*c_in
            X00=np.reshape(X0[:n],(n/c_in,c_in,H,W)) # head
            X01=np.reshape(X0[-n-1:-1],(n/c_in,c_in,H,W)) # head
            X0=np.concatenate((X00,X01),axis=0)
            Y0=np.zeros((X0.shape[0],1,H,W),dtype='uint8')
            X.append(X0)
            Y.append(Y0)
    X=np.vstack(X)        
    Y=np.vstack(Y).astype('uint8')        
    ff_dsbnc_r=h5py.File(path2dsbnc_train,'w-')
    ff_dsbnc_r['X_train']=X
    ff_dsbnc_r['Y_train']=Y
    ff_dsbnc_r.close()
    print 'hdf5 saved!'
...
for epoch in range(params_train['nbepoch']):
    ...
    for k in range(0,X_train.shape[0],bs2):
        ...
        #X_batch=np.append(X_train[k:k+bs2],X_train2[k:k+bs2],axis=0)
        #Y_batch=np.append(Y_train[k:k+bs2],Y_train2[k:k+bs2],axis=0)
    ...
    score_test=[]
    for k2 in range(0,X_test.shape[0],8):
        tmp=model.evaluate(*utils.preprocess_XY(X_test[k2:k2+8],Y_test[k2:k2+8],param_prep),verbose=0)
    ...
    #score_train=model.evaluate(X_batch, Y_batch[:,0,:][:,np.newaxis,:],verbose=0)
