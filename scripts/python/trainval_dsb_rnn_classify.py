...
experiment='dsb_rnn_classify'+'_hw_'+str(h)+'by'+str(w)
...
def extract_dsb(ids):
    ...
    for id in ids:    
        ...
        # obtain nodules 
        Yp_seg=seg_model.predict(X1)
        #Yp_seg=np.append(Yp_seg,X1,axis=1)
        Y_seg.append(Yp_seg)
...
def pad4rnn(X,timestep):
    ...
    for k in range(len(X)):
        ...
        if X[k].shape[0]>timestep:
            ...
            Xt[k]=np.append(X[k],np.zeros((timestep-X[k].shape[0],1,h,w),'float32'),axis=0)
...
for epoch in range(params_train['nbepoch']):
    ...
    #for t1 in range(0,len(trn_ids),bs):
        #trn_id_batch=trn_ids[t1:t1+bs]                
        #print t1
    
    trn_id_batch=random.sample(trn_ids,4*bs)
        
    # extract nodule masks 
    X_batch,y_batch=extract_dsb(trn_id_batch)    
            
    # fit model to data
    model_rnn.fit(X_batch, y_batch, nb_epoch=1, batch_size=bs,verbose=0,shuffle=True)    

    # evaluate on test and train data
    X_test,y_test=extract_dsb(random.sample(val_ids,bs)) 
    score_test=model_rnn.evaluate(X_test,y_test,verbose=0)
    
    score_train=model_rnn.evaluate(X_batch,y_batch,verbose=0)
