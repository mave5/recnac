...
subset_list=glob(path2luna+'*.hdf5')
...
def load_data(subsets):
    ...
    for ss in subsets:
        ...
        for k in ff.keys():
            ...
            nz_y_i=len(nz_Y)-int(len(nz_Y)/3)*3
            nz_Y=nz_Y[nz_y_i:]
            XY=XY[:,nz_Y]
            X0=XY[0]
            Y0=XY[1]
            X0=np.reshape(X0,(X0.shape[0]/3,3,X0.shape[1],X0.shape[2]))
            Y0=np.reshape(Y0,(Y0.shape[0]/3,3,Y0.shape[1],Y0.shape[2]))
            X.append(X0)
            Y.append(Y0)
        ff.close()    
    X=np.vstack(X)        
...
Y_train=Y_train[:,c_out,:][:,np.newaxis,:]
...
n1=utils.disp_img_2masks(X,Y_pred,Y,4,4,0)
plt.show()
#%%
# deply on test data
path2luna='./output/data/luna/test_luna_nodules.hdf5'
...
Y_pred=array_resize(Y_pred,(256,256))
