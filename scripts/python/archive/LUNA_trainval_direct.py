...
h,w=128,128
experiment='direct'+'_hw_'+str(h)+'by'+str(w)
...
def model(params):
    ...
    for k in range(1,N):
        ...
        model.add(Convolution2D(C1, 3, 3, subsample=(2,2), activation='relu', border_mode='same'))              
    ...
    for k in range(1,N):
        C2=np.min([2**(N-k)*C,512])
        model.add(UpSampling2D(size=(2, 2)))
        model.add(Convolution2D(C2, 3, 3, activation='relu', border_mode='same'))
...
for e in range(params_train['nbepoch']):
    ...
    # fit model
    model.fit(X_train, Y_train.astype('float32'), validation_data=(X_test, Y_test.astype('float32')), nb_epoch=1, batch_size=bs,verbose=0,shuffle=True,callbacks=[checkpoint])
    
    # evaluate on test and train data
    score_test=model.evaluate(X_test, Y_test.astype('float32'),verbose=0)
    score_train=model.evaluate(X_train, Y_train.astype('float32'),verbose=0)
    if params_train['loss']=='dice': 
